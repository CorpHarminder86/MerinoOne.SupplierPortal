...
public static class IntegrationCatalog
{
    ...
    public static readonly IReadOnlyList<IntegrationEndpointDocDto> All = new[]
    {
        ...
        new IntegrationEndpointDocDto("Purchase orders", "Integration.Inbound.Po", "POST", $"{Base}/purchase-orders", true,
            "Create/upsert Purchase Orders (+ lines) for the resolved company. Owning supplier resolved by erpSupplierCode (matched on Supplier.ErpCode) OR supplierCode (matched on Supplier.SupplierCode) — when both are sent erpSupplierCode wins; at least one is required. Each line carries a warehouse ERP code (REQUIRED, <=50, R13) that resolves to an active, non-base CompanyAddress by ErpCode within the PO's company; on resolution the line stores the raw code, the address FK + a point-in-time snapshot, and if ANY line's warehouse does not resolve the WHOLE PO row is HARD-FAILED. warehouse is part of the canonical idempotency digest, so a PO re-pushed only to deliver a changed warehouse is not deduped away. erpStatus is the raw ERP status, stored for tracking only. poOrigin (<=50) is the raw ERP-owned PO origin, stored verbatim on the header for reference only. Currency/term/item/tax resolved by code; an UNRESOLVED paymentTermCode/deliveryTermCode/itemCode/taxCode/orderUnit does NOT fail the row — the PO saves with the FK null, the row result carries a 'Warning — unresolved master codes' advisory and a Failed Sync Log row records the drift. itemCode is REQUIRED per line (<=50); warehouse is REQUIRED per line (<=50); erpStatus <=50; term codes <=20; orderUnit/taxCode <=20. Line storage key is positionNo (sequenceNo accepted but always stored as 1; lines folded by positionNo). orderQty and additionalQty are mutually exclusive: orderQty>0 REPLACES the qty, additionalQty!=0 ADDS a signed delta (may be negative) to the current qty.",
            Json(new { companyCode = "3000", orders = new[] { new { poNumber = "PO-3000-000001", erpSupplierCode = "ERP-S0001", supplierCode = "S0001", erpStatus = "Released", poOrigin = "LN", poDate = "2026-06-23", poType = "Material", poStatus = "Released", currencyCode = "INR", paymentTerms = "Net 30", deliveryTerms = "FOB", paymentTermCode = "NET30", deliveryTermCode = "FOB", notes = "Sample PO", erpSyncId = "LN-PO-1", lines = new[] { new { positionNo = 10, sequenceNo = 1, itemCode = "ITM-00001", itemDescription = "Sample item", warehouse = "DC-3000-01", orderUnit = "KG", orderQty = 100.0, additionalQty = 0.0, priceUnit = 50.0, price = 5000.0, discountPct = 0.0, discountAmount = 0.0, deliveryDate = "2026-07-15", taxCode = "GST18", taxDescription = "GST 18%" } } } } })),
