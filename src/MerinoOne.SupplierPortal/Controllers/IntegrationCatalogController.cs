...
public static class IntegrationCatalog
{
    ...
    public static readonly IReadOnlyList<IntegrationEndpointDocDto> All = new[]
    {
        ...
        new IntegrationEndpointDocDto("ERP ack", "Integration.Inbound.ErpAck", "POST", $"{Base}/erp-ack", false,
            "ERP acknowledgement + erpCode write-back for a Portal->ERP transaction (tenant-scoped; resolves portalRef -> outbox row). R8/R11.2: InvoicePost acks may carry the ERP composite key (erpCompany/erpTransactionType/erpDocumentNo) written to the Invoice for its Infor IDM eligibility gate; on AsnPost the composite is accepted but not stored — the ASN gate keys on erpCode alone, and erpCompany always equals the company code the portal sent on the request.",
            Json(new { acks = new[] { new { transactionType = "InvoicePost", portalRef = "<deterministic-outbox-key>", success = true, erpCode = "INV-LN-0001", message = (string?)null, erpCompany = "2000", erpTransactionType = "1DS", erpDocumentNo = "100000001" } } })),
        ...
        new IntegrationEndpointDocDto("Purchase orders", "Integration.Inbound.Po", "POST", $"{Base}/purchase-orders", true,
            "Create/upsert Purchase Orders (+ lines) for the resolved company. Owning supplier resolved by erpSupplierCode (matched on Supplier.ErpCode) OR supplierCode (matched on Supplier.SupplierCode) — when both are sent erpSupplierCode wins; at least one is required. shipToAddress (REQUIRED, R5) is the ERP ship-to code resolved to CompanyAddress.erpCode within the PO's company; on resolution the PO stores the ship-to FK + a point-in-time snapshot, and an unresolvable code HARD-FAILS the row. warehouse (REQUIRED, <=50, R11) is the receiving warehouse code — free text stored verbatim on the header (no warehouse master, so an unknown code is accepted and never fails the row); it becomes the single-warehouse grouping key for every ASN raised against the PO and is forwarded to LN in the ASN payload. It is part of the canonical idempotency digest, so a PO re-pushed only to deliver a changed warehouse is not deduped away; POs ingested before R11 keep a null warehouse until LN re-pushes them. erpStatus is the raw ERP status, stored for tracking only. poOrigin (<=50) is the raw ERP-owned PO origin, stored verbatim on the header for reference only. Currency/term/item/tax resolved by code; an UNRESOLVED paymentTermCode/deliveryTermCode/itemCode/taxCode/orderUnit does NOT fail the row — the PO saves with the FK null, the row result carries a 'Warning — unresolved master codes' advisory and a Failed Sync Log row records the drift. itemCode is REQUIRED per line (<=50); erpStatus <=50; term codes <=20; orderUnit/taxCode <=20. Line storage key is positionNo (sequenceNo accepted but always stored as 1; lines folded by positionNo). orderQty and additionalQty are mutually exclusive: orderQty>0 REPLACES the qty, additionalQty!=0 ADDS a signed delta (may be negative) to the current qty.",
            Json(new { companyCode = "3000", orders = new[] { new { poNumber = "PO-3000-000001", erpSupplierCode = "ERP-S0001", supplierCode = "S0001", shipToAddress = "DC-3000-01", warehouse = "WH-3000-01", erpStatus = "Released", poOrigin = "LN", poDate = "2026-06-23", poType = "Material", poStatus = "Released", currencyCode = "INR", paymentTerms = "Net 30", deliveryTerms = "FOB", paymentTermCode = "NET30", deliveryTermCode = "FOB", notes = "Sample PO", erpSyncId = "LN-PO-1", lines = new[] { new { positionNo = 10, sequenceNo = 1, itemCode = "ITM-00001", itemDescription = "Sample item", orderUnit = "KG", orderQty = 100.0, additionalQty = 0.0, priceUnit = 50.0, price = 5000.0, discountPct = 0.0, discountAmount = 0.0, deliveryDate = "2026-07-15", taxCode = "GST18", taxDescription = "GST 18%" } } } } })),
