...
Warehouse (R11): warehouse is the receiving warehouse CODE and is REQUIRED — a payload omitting it is rejected with 400 for the whole batch. It is free text stored verbatim on the PO header (ERP-owned, overwritten on the next inbound sync); there is NO warehouse master, so an unknown code is accepted and never fails the row. It flows onto every ASN raised against the PO as the single-warehouse grouping key (an ASN may not mix warehouses) and is forwarded to LN in the ASN payload. It is part of the canonical idempotency digest, so a PO re-pushed only to deliver a changed warehouse is NOT deduped away. Existing POs ingested before this field keep a null warehouse until LN re-pushes them — there is no backfill.
Field caps (400 on violation): warehouse REQUIRED <=50, erpStatus <=50, poOrigin <=50, paymentTermCode/deliveryTermCode <=20; per line itemCode REQUIRED <=50, orderUnit <=20, taxCode <=20.
